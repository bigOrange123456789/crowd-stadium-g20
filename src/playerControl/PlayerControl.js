...
function PlayerControl0(camera){
    ...
    myMouseManager.dragMouse=function (dx,dy) {
        ...
        if(scope.rotation_index==1){
            scope.rotation_index=2
        }else{
            scope.rotation_index=1
        }
        scope.rotation1(-0.015*dx);//偏航角
        scope.rotation2(-0.015*dy);//俯仰角
    }
    ...
    this.rotation1=function(step){//水平旋转
        ...
        this.camera.matrixWorld.compose(this.camera.position, this.camera.quaternion, this.camera.scale);
    }
